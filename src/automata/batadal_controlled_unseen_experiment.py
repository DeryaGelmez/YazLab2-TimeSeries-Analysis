...
def create_controlled_unseen_test_patterns(
    ...
    for idx in selected_indices:
        ...
        attempts = 0
        while new_pattern in train_states and attempts < 20:
            new_pattern = create_unseen_pattern(original_pattern, alphabet_size)
            attempts += 1
...
def main():
    print("BATADAL Kontrollü Unseen Deneyi Başladı (5 seed)")
    ...
    results_df.to_csv(LOGS_DIR / "batadal_controlled_unseen_results.csv", index=False)
    pd.DataFrame(all_unseen_records).to_csv(
        LOGS_DIR / "batadal_controlled_unseen_mapping.csv", index=False
    )
    pd.DataFrame([summary]).to_csv(
        LOGS_DIR / "batadal_controlled_unseen_summary.csv", index=False
    )

    print("\nBATADAL Kontrollü Unseen Deneyi Tamamlandı")
